...
class Users {
    ...
    function obtenerReporteGlobal($fechaInicial,$fechaFinal,$id_sucursal,$empresa,$usuario,$doctor,$estatus,$tipo_pago){
        ...
        $sql = "SELECT  pa.email as correo,o.telefono,GROUP_CONCAT(est.clase) as clases, GROUP_CONCAT(ece.tipo_descuento) as tipos_descuento_clase,GROUP_CONCAT(ece.porcentaje_descuento) as montos_descuento_clase,e.porcentaje as porcentaje_descuentoc,o.tipo_cliente,dep.departamento as nombre_departamento,clasee.nombre as nombre_clasee, o.tipo_orden, GROUP_CONCAT(paq.nombre) as nombre_paquete,GROUP_CONCAT(ref.codigo) as referencias, o.sucursal_maquila, GROUP_CONCAT(oe.id_estudio) as id_estudios,o.cancelado, o.consecutivo,e.nombre as empresa, o.credito,CONCAT(pa.paterno,' ',pa.materno,' ',pa.nombre) as paciente,if(o.id_doctor is null,
CONCAT(o.nombre_doctor,'(nr)'),CONCAT(d.alias,'-',d.apaterno,' ',d.amaterno,' ',d.nombre)) as doctor, o.fecha_registro, u.nombre as usuario, GROUP_CONCAT(ce.no_estudio) as codigos_estudio,
GROUP_CONCAT(ce.nombre_estudio) as nombres_estudio,GROUP_CONCAT(oe.precio_neto_estudio) as precios_netos,GROUP_CONCAT(oe.precio_publicoh) as preciosp_estudio,p.pago, 
o.importe,o.importe-o.saldo_deudor as acuenta,o.saldo_deudor,GROUP_CONCAT(DISTINCT fp.descripcion) as forma_pago,GROUP_CONCAT(sec.seccion) as secciones,o.tipo_orden,if(o.sucursal_maquila>0,'maquila','-') as maquila,suc.nombre as nombre_sucursal from orden o 
        left join pago p on o.id=p.id_orden 
