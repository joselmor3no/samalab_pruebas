...
class FacturacionModel {
    ...
    public function listaOrdenesIndividualesM($mes_busqueda){
    	$sql="SELECT p.id as id_paciente,em.nombre as empresa_nombre,o.id_empresa,o.fecha_registro, o.consecutivo as orden_c,o.id as orden_id,concat(p.paterno,' ',p.materno,' ',p.nombre) as nombre,o.importe FROM orden o INNER JOIN paciente p on o.id_paciente=p.id left join empresa em on em.id=o.id_empresa 
		   where (o.id_sucursal=".$_SESSION['id_sucursal']." and YEAR(o.fecha_registro)=YEAR(CURDATE()) and MONTH( o.fecha_registro) = ".$mes_busqueda.") and o.id_factura IS NULL  order by o.fecha_registro desc";
    ...
    public function guardarDatosFacturaPacienteM(){
    	$sql = 'INSERT INTO datos_fiscales_paciente (id_paciente, rfc, nombre_fiscal, direccion_fiscal, codigo_postal, correo, condiciones_pago, metodo_pago, forma_pago, uso_cfdi,cfdi_relacionado, observaciones) 
    	VALUES (
    	'.$_REQUEST['m_id_paciente'].',
    	"'.strtoupper($_REQUEST['m_rfc']).'",
    	"'.$_REQUEST['m_nombre_fiscal'].'",
    ...
    public function actualizaDatosFacturaPacienteM(){
        ...
        nombre_fiscal="'.$_REQUEST['m_nombre_fiscal'].'",
    ...
    public function guardarDatosFacturaMasivaM(){
        ...
        $sql = 'INSERT INTO datos_fiscales_forma_masiva (rfc, nombre_fiscal, direccion_fiscal, codigo_postal, correo, condiciones_pago, metodo_pago, forma_pago, uso_cfdi,cfdi_relacionado, observaciones,usar_descripcion,descripcion) 
        VALUES (
        "'.strtoupper($_REQUEST['fm_rfc']).'",
        "'.$_REQUEST['fm_nombre_fiscal'].'",
    ...
    public function actualizaDatosFacturaMasivaM(){
        ...
        nombre_fiscal="'.$_REQUEST['fm_nombre_fiscal'].'",
    ...
    public function itemsOrdenCFDIM($miva){
    ...
    public function guardaCFDIM(){
        ...
        if($_REQUEST['tipo']=='individual'){
           ...
           $update="UPDATE orden SET id_factura='".$id."'  WHERE id=".$_REQUEST['lista_ordenes']; 
           $sqlDFP="SELECT dfp.* from datos_fiscales_paciente dfp inner join paciente p on p.id=dfp.id_paciente inner join orden o on o.id_paciente=p.id where o.id=".$_REQUEST['lista_ordenes']." order by o.fecha_registro DESC";
...
//-----------------Informacion para reporte de facturas
    ...
    public function obtenerCFDIEmpresaM(){
        $sql="SELECT rfc,nombre_fiscal,email,direccion,cp,condiciones_pago,metodo_pago,forma_pago,uso_cfdi,observaciones,usar_descripcion,descripcion FROM `empresa` WHERE id=".$_REQUEST['id_empresa']."";
        return $this->conexion->getQuery($sql);
    }
    ...
    public function precancelarFacturasM($id_cfdi){
        ...
        for($i=0;$i<count($arrayOrdenes);$i++){
            $updt="UPDATE orden SET id_factura=null where (consecutivo=".$arrayOrdenes[$i]." or id=".$arrayOrdenes[$i].") and id_factura='".$id_cfdi."'";
