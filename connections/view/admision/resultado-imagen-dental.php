    ...
    <section class="content">
        ...
        <div class="card">
            <div class="card-header">
                <form class="needs-validation" action="resultados-imagen" method="post" novalidate="">
                    <div class="row">
                        <div class="col-md-3">
                    ...
                    <tbody>
                        ...
                        foreach ($datos as $row) {
                            ...
                            <tr>
                                ...
                                <td><?= $row->cerrado == 1 ? "<span class='text-primary'>REPORTADO</span>" : "<span class='text-danger'>SIN REPORTAR</span>" ?></td>
                                ...
                                <td> 
                                    ...
                                    if ($row->local =='N') {
                                        ...
                                        ?>
                                        <div class="row">
                                            <?php if($row->cerrado==1):?>
                                                <button class="btn btn-sm btn-warning rounded-circle m-1" onclick="<?= "window.open('https://reporte.imagen.connectionslab5.net/sama/reporte/imprimir_rdcm.php?dcm=".$row->id_dcm."&p=1')"?>"  title="Imprimir">
                                                    <i class="far fa-file-pdf"></i>
                                                </button>
                                            <?php endif;?>
                                            <button class="btn btn-sm btn-info rounded-circle m-1" onclick="<?= "window.open('../Pacientes/redirige_visor.php?nombre=".$row->ruta."&cerrado=".$row->cerrado."&prefijo_sucursal=".$sucursal->prefijo_imagen."&id_dcm=".$row->id_dcm."&usuario=1&ruta=sama')"?>"  data-toggle="tooltip" title="Ver Imagenes">
                                            ...
                                            <button class="btn btn-sm btn-dark rounded-circle m-1 mail" data-paciente="<?= $row->paciente ?>" data-correo="<?= $row->correo ?>" data-id="<?= $row->id ?>" data-expediente="<?= $row->expediente ?>"  data-toggle="tooltip" title="Mail">
